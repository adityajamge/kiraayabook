    ...
    <>
      ...
      <div className="lg:hidden">
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-2xl font-bold dark:text-white">{t('rent.title')}</h1>
        ...
        {!loading && records.length > 0 && (
          <div className="grid grid-cols-2 gap-3 mb-4">
            <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl p-4 shadow-sm">
              ...
              <p className="text-xs text-gray-500">{t('rent.collected')}</p>
            ...
            <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl p-4 shadow-sm">
              ...
              <p className="text-xs text-gray-500">{t('rent.pending')}</p>
        ...
        {loading ? (
          ...
          <p className="text-center py-12 text-gray-400 text-sm">{t('rent.noRecords')}</p>
        ) : (
                ...
                <div key={r.id} className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl p-4 shadow-sm">
                  <div className="flex items-center gap-3 mb-2.5">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500">Room {tenant ? roomMap[tenant.room_id] ?? '—' : '—'} · {fmt(r.amount)}</p>
                    </div>
                    <span className={`text-xs font-semibold px-2.5 py-1 rounded-full shrink-0 ${
                      ...
                      {r.status === 'paid' ? t('rent.paidStatus') : overdue ? t('rent.overdue') : t('rent.pendingStatus')}
                    </span>
                  </div>
      ...
      <div className="hidden lg:block">
        <div className="flex items-center justify-between gap-3 mb-6">
          <div>
            <h1 className="text-2xl font-bold dark:text-white">{t('rent.titleDesktop')}</h1>
            <p className="text-gray-500 text-sm mt-0.5">{t('rent.subtitle')}</p>
          </div>
        ...
        {!loading && records.length > 0 && (
          <div className="grid grid-cols-2 gap-3 mb-5">
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl px-5 py-4">
              <p className="text-xs text-gray-400 mb-1">{t('rent.collected')}</p>
              <p className="text-xl font-bold text-green-600">{fmt(paid)}</p>
            </div>
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl px-5 py-4">
              <p className="text-xs text-gray-400 mb-1">{t('rent.pending')}</p>
        ...
        {loading ? <TableSkeleton cols={9} /> : records.length === 0 ? (
          <div className="text-center py-12 text-gray-400 text-sm">{t('rent.noRecords')}</div>
        ) : (
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl overflow-x-auto">
            <table className="w-full min-w-200 text-sm">
              <thead>
                <tr className="border-b border-gray-100 dark:border-gray-700">
                  {[t('rent.tenantHeader'), t('rent.roomHeader'), t('rent.periodHeader'), t('rent.amountHeader'), t('rent.dueDateHeader'), t('rent.paidDateHeader'), t('rent.modeHeader'), t('rent.statusHeader'), ''].map((h, i) => (
                ...
                {records.map((r) => {
                  ...
                  return (
                    <tr key={r.id} className="border-b border-gray-50 dark:border-gray-800/60 hover:bg-gray-50 dark:hover:bg-gray-800">
                      <td className="px-5 py-4 font-medium dark:text-white">{tenant?.name ?? '—'}</td>
                      <td className="px-5 py-4 text-gray-500">{tenant ? roomMap[tenant.room_id] ?? '—' : '—'}</td>
                      ...
                      <td className="px-5 py-4">
                        <span className={r.status === 'paid'
                          ...
                          {r.status === 'paid' ? t('rent.paid') : t('rent.pending')}
                        </span>
                      </td>
                      <td className="px-5 py-4">
                        <button
